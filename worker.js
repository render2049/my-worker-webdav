// worker.js
// ==========================================
// Cloudflare Worker + KV WebDAV 简易网盘
// 直接上传到 GitHub，然后 Cloudflare 上传部署即可
// 功能：上传/下载/删除文件 + KV 存储 + 前端页面
// ==========================================
...
async function upload() {
  ...
  bar.style.width = "0%"; bar.innerText="0%"; msg.innerText="";
  ...
  reader.onload = async () => {
    ...
    try {
      ...
      xhr.onload = () => {
        if(xhr.status === 200){
          bar.style.width = "100%"; bar.innerText = "完成";
          msg.innerText = "上传成功 ✅";
        } else {
      ...
      xhr.onerror = ()=>{ bar.style.width="0%"; bar.innerText="0%"; msg.innerText="上传失败 ❌"; }
      xhr.send(chunk);
    } catch(e){
      bar.style.width="0%"; bar.innerText="0%"; msg.innerText="上传失败 ❌";
    }
  };
